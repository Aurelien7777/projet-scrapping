...
for urls in all_urls_category_sans_index:
    ...
    with open(f"data_book_category_{all_category_name[compteur]}.csv", "w", encoding="utf-8", newline="") as csv_file:
            ...
            if number_of_page > 1:
                ...
                if response.ok: #Vérification de la connexion à la page
                    ...
                    for book in all_book_on_page: 
                        ...
                        if response_book.ok:
                            print("Réponse à la requête:", response_book.ok)
                ...
                response.encoding = 'utf-8'
                print("L'URL de la catégorie est: ",url_category)
                if response.ok: #Vérification de la connexion à la page
                    soup_category = BeautifulSoup(response.text, 'html.parser')
                    #Récupération de tous les livres présents sur la page
                    all_book_on_page = soup_category.find_all('li', class_="col-xs-6 col-sm-4 col-md-3 col-lg-3") 
                    ...
                    for book in all_book_on_page: #Récupération de chaque livre présent sur la page
                        ...
                        if response_book.ok:
                            ...
                            print()
                            print("La réponse à la requête", response_book)
